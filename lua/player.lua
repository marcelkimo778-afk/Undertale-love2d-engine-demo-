...
function Player:keypressed(key)
    if (key == "c" and substate ~= "subinfo") or ((substate == "submenu" or substate == "subinfo") and key == "x") then
    if substate == "Play" then
        substate = "submenu"
        move:play()
...
function Player:draw()
    ...
    if substate == "submenu" or substate == "subinfo" then
        ...
        -- Decide box order based on player position
        local box1Y, box2Y
        if self.y > 240 then
            -- Player near top: info box flips to bottom
            box1Y = 320
            box2Y = 165
        else
            -- Normal: info box stays on top
            box1Y = 50
            box2Y = 165
        end
...
--BOX 1
        ...
        love.graphics.rectangle("fill", 36, box1Y, 135, 102)

        love.graphics.setColor(1, 1, 1)
        love.graphics.setLineWidth(6)
        love.graphics.rectangle("line", 36, box1Y, 135, 102)


-- BOX 2
        love.graphics.setColor(0, 0, 0)
        love.graphics.rectangle("fill", 36, box2Y, 135, 142)

        love.graphics.setColor(1, 1, 1)
        love.graphics.setLineWidth(6)
        love.graphics.rectangle("line", 36, box2Y, 135, 142)
...
--BOX 1 info 
        ...
        love.graphics.print("LV " .. stat.lv .. "\nHP " .. soul.hp .. "/" .. soul.maxhp .. "\nG   " .. stat.G, 48, box1Y + 45)
        local font = love.graphics.newFont("Assets/font/dete.ttf", 16 * 2)
        love.graphics.setFont(font)
        love.graphics.print(stat.name, 48, box1Y + 10)

--BOX 2 info (offsets relative to box2Y, using original offsets from 165)
        love.graphics.print("ITEM", 78, box2Y + 22)
        love.graphics.print("STAT", 78, box2Y + 58)
        love.graphics.print("CELL", 78, box2Y + 94)

--BOX INFO subinfo
    if substate == "subinfo" then
        love.graphics.setColor(0, 0, 0)
        love.graphics.rectangle("fill", 192, 50, 330, 415)

        love.graphics.setColor(1, 1, 1)
        love.graphics.setLineWidth(6)
        love.graphics.rectangle("line", 192, 50, 330, 415)

        love.graphics.print("\"" .. stat.name .. "\" \n \n" .. "LV " .. stat.lv .. "\nHP " .. soul.hp .. "/" .. soul.maxhp, 212, 84)
    end 
    end
end
