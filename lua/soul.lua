...
function soul.load()
    ...
    soul.name = "Kimo"
    soul.lv = 19
    soul.hpSprite = love.graphics.newImage("Assets/sprites/UI/hp.png")
    soul.krSprite = love.graphics.newImage("Assets/sprites/UI/kr.png")
    soul.barX = 230
    soul.barY = 402
    soul.barH = 23
    soul.barUnit = 1.35
...
local function getDisplayHP()
    return clamp(math.floor(soul.hp + soul.kr), 0, soul.maxhp)
end

local function getYellowHP()
    return clamp(math.floor(soul.hp), 0, soul.maxhp)
end

local function getPurpleHP()
    return clamp(getDisplayHP() - getYellowHP(), 0, soul.maxhp)
end

local function reserveKRSpace()
    local maxYellow = soul.maxhp - soul.kr

    if maxYellow < 0 then
        maxYellow = 0
    end

    if soul.hp > maxYellow then
        soul.hp = maxYellow
    end
end

local function updateKR(dt)
    soul.hp = math.min(soul.hp, soul.maxhp)
    soul.kr = math.max(soul.kr, 0)

    if soul.kr > soul.maxhp - 1 then
        soul.kr = soul.maxhp - 1
    end

    reserveKRSpace()

    if soul.kr > 0 then
        soul.krtimer = soul.krtimer - dt * (soul.kr / 20)

        if soul.krtimer <= 0 then
            soul.kr = soul.kr - 1
            soul.krtimer = soul.krdelay
        end
    else
        soul.kr = 0
        soul.krtimer = 0
    ...
    if soul.hp < 1 and soul.kr > 0 then
        soul.hp = 1
    elseif soul.hp < 0 then
        soul.hp = 0
    end

    soul.kr = clamp(math.floor(soul.kr), 0, soul.maxhp)
    reserveKRSpace()
end
...
local function drawHealthUI()
    ...
    local barWidth = soul.maxhp * soul.barUnit

    local valueText
    if displayHP < 10 then
        valueText = "      " .. displayHP .. " / " .. soul.maxhp
    else
        valueText = "     " .. displayHP .. " / " .. soul.maxhp
    end
    ...
    if soul.kr > 0 then
        valueColor = {0.75, 0.1, 1, 1}
    end

    love.graphics.setFont(soul.UIfont)

    drawOutlinedText(soul.name .. "  lV " .. soul.lv, 20, 406, {1, 1, 1, 1})
    love.graphics.setColor(1, 1, 1, 1)
    love.graphics.draw(soul.hpSprite, 190 +30, 404)
    love.graphics.draw(soul.krSprite, soul.barX + barWidth + 13 +24, 409)
    drawOutlinedText(valueText, soul.barX + barWidth + 12 +30, 406, valueColor)

    love.graphics.setColor(1, 0, 0, 1)
    love.graphics.rectangle("fill", soul.barX+ 30, soul.barY, barWidth, soul.barH)

    if displayHP > 0 then
        love.graphics.setColor(0.75, 0.1, 1, 1)
        love.graphics.rectangle("fill", soul.barX +30, soul.barY, displayHP * soul.barUnit, soul.barH)
    end

    if yellowHP > 0 then
        love.graphics.setColor(1, 1, 0, 1)
        love.graphics.rectangle("fill", soul.barX +30, soul.barY, yellowHP * soul.barUnit, soul.barH)
    end

    love.graphics.setLineWidth(1)
    love.graphics.setColor(0, 0, 0, 1)
    love.graphics.rectangle("line", soul.barX +30, soul.barY, barWidth, soul.barH)
end
...
local function gameover(dt)
    ...
    if soul.hp <= 0 and soul.kr == 0 then
...
drawShards()
    ...
    if game.state ~= "Gameover" and substate ~= "submenu" then
    drawHealthUI()
    end
end
